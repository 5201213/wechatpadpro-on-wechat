...
class DpBotMessage(ChatMessage):
    def __init__(self, msg, client: DPBotClient = None):
        super().__init__(msg)
        self.msg = msg
        ...
        if 'Data' in msg:
            ...
            logger.warning(f"[dpbot] Unknown message format, missing expected fields")
            logger.debug(f"[dpbot] Available keys: {list(msg.keys())}")
        ...
        if not self.msg_data or not self.msg_data.get('NewMsgId'):
            logger.warning(f"[dpbot] Invalid message data or missing NewMsgId")
            logger.debug(f"[dpbot] msg_data: {self.msg_data}")
        ...
        self.is_group = True if "@chatroom" in self.from_user_id else False
        ...
        if self._is_non_user_message(self.msg_source, self.from_user_id, content, msg_type):
            ...
            logger.debug(f"[dpbot] detected non-user message from {self.from_user_id}: {self.content}")
            return
        ...
        if msg_type == 1:  # Text message
            ...
            if any(note in content for note in notes_join_group):
                try:
                    ...
                    logger.debug(f"[dpbot] parsed group system message: {self.content} "
                                f"type: {content_type} user: {target_nickname} ({target_username})")
                    
                except ET.ParseError as e:
                    logger.error(f"[dpbot] Failed to parse group system message XML: {e}")
                    self.content = content
                except Exception as e:
                    logger.error(f"[dpbot] Unexpected error parsing group system message: {e}")
                    self.content = content
        elif msg_type == 10002:  # 系统消息（非群聊）
            content_dict = self.msg_data.get('Content', {})
            content = content_dict.get('str', content_dict.get('string', ''))
            logger.debug(f"[dpbot] detected system message (non-group): {content}")
            ...
            content_dict = self.msg_data.get('Content', {})
            content = content_dict.get('str', content_dict.get('string', ''))
            logger.debug(f"[dpbot] detected friend request message: {content}")
            # 好友请求消息设置为文本类型但不处理
            self.ctype = ContextType.TEXT
            self.content = content
            ...
            content_dict = self.msg_data.get('Content', {})
            content = content_dict.get('str', content_dict.get('string', ''))
            logger.debug(f"[dpbot] detected system notification message: {content}")
            # 系统提示消息设置为文本类型但不处理
            self.ctype = ContextType.TEXT
            self.content = content
        elif msg_type == 47:
            ...
            logger.warning(f"[dpbot] Unsupported message type: Type:{msg_type}, will process as text")
            self.ctype = ContextType.TEXT
            content_dict = self.msg_data.get('Content', {})
            self.content = content_dict.get('str', content_dict.get('string', ''))
        ...
        if "@chatroom" in self.other_user_id:
            ...
            try:
                ...
                if cached_group_name:
                    ...
                    logger.debug(f"[dpbot] 从数据库获取群名称: {self.other_user_id} -> {cached_group_name}")
                else:
                    # 如果数据库中没有，使用API获取
                    if self.client:
                        try:
                            logger.debug(f"[dpbot] 尝试获取群聊信息: {self.other_user_id}")
                            ...
                            if contact_info_response.get('Code') == 200 and contact_info_response.get('Data'):
                                ...
                                if contact_list:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"[dpbot] 保存群名称到数据库失败: {e}")
                                else:
                                    self.other_user_nickname = self.other_user_id
                            else:
                                self.other_user_nickname = self.other_user_id
                        except Exception as e:
                            logger.warning(f"[dpbot] 获取群聊信息失败: {e}")
                            self.other_user_nickname = self.other_user_id
                    else:
                        self.other_user_nickname = self.other_user_id
            except Exception as e:
                logger.debug(f"[dpbot] 从数据库获取群名称失败: {e}")
                self.other_user_nickname = self.other_user_id
        else:
            # 私聊 - 先尝试从群成员数据库获取昵称
            try:
                ...
                if cached_nickname:
                    ...
                    logger.debug(f"[dpbot] 从数据库获取用户昵称: {self.other_user_id} -> {cached_nickname}")
                else:
                    # 如果数据库中没有，使用API获取
                    if self.client:
                        try:
                            logger.debug(f"[dpbot] 尝试获取用户信息: {self.other_user_id}")
                            contact_info_response = self.client.get_contact_details_list([], [self.other_user_id], user_key=None)
                            ...
                            logger.warning(f"[dpbot] 获取用户信息失败: {e}")
                            self.other_user_nickname = self.other_user_id
                    else:
                        self.other_user_nickname = self.other_user_id
            except Exception as e:
                logger.debug(f"[dpbot] 从数据库获取用户昵称失败: {e}")
                self.other_user_nickname = self.other_user_id
        ...
        if self.is_group:
            ...
            try:
                ...
                logger.warning(f"[dpbot] Failed to get group member display name: {e}")
    ...
    def download_voice(self):
        ...
        try:
            if not self.client:
                logger.error("[dpbot] 没有客户端实例，无法下载语音")
                return
            ...
            content_xml = content_dict.get('str', content_dict.get('string', ''))
            if not content_xml:
                logger.error("[dpbot] 没有找到语音XML内容")
                return
            ...
            try:
                ...
                if not content_xml.startswith('<'):
                    logger.error(f"[dpbot] 语音XML格式不正确: {content_xml[:50]}...")
                    return
                ...
                if voicemsg is None:
                    logger.error("[dpbot] XML中没有找到voicemsg元素")
                    return
                ...
                if not bufid or not length or not length.isdigit() or int(length) <= 0 or not new_msg_id:
                    logger.error(f"[dpbot] 语音参数无效: bufid={bufid}, length={length}, new_msg_id={new_msg_id}")
                    return
                ...
                if voice_response.get("Code") == 200:
                    ...
                    if voice_base64:
                        ...
                        logger.info(f"[dpbot] SILK文件保存成功: {silk_file_path}")
                        ...
                        if os.path.exists(mp3_file_path):
                            ...
                            logger.info(f"[dpbot] 语音处理完成: {mp3_file_path}, 时长: {target_duration}秒")
                    else:
                        logger.error("[dpbot] API响应中没有Base64语音数据")
                else:
                    logger.error(f"[dpbot] 语音下载API失败: {voice_response}")
            
            except ET.ParseError as e:
                logger.error(f"[dpbot] 解析语音XML失败: {e}, 内容: {content_xml[:50]}...")
                
        except Exception as e:
            logger.error(f"[dpbot] 语音文件下载异常: {e}", exc_info=True)
    ...
    def download_image(self):
        ...
        try:
            ...
            if os.path.exists(self.content):
                logger.info(f"[dpbot] 图片文件已存在: {self.content}")
                return
            ...
            for file_type, cdn_url in download_urls:
                ...
                if result and isinstance(result, dict) and result.get("Code") == 200:
                    ...
                    if image_data and self._save_image_data(image_data):
                        logger.info(f"[dpbot] 图片下载成功: {self.content}")
                        return
                
        except Exception as e:
            logger.error(f"[dpbot] 图片下载异常: {e}")
    ...
    def download_refer_image(self):
        ...
        try:
            ...
            if not cdn_info.get('aes_key'):
                logger.error("[dpbot] 引用图片CDN信息提取失败")
                return
            ...
            # 检查文件是否已存在，避免重复下载
            if os.path.exists(self.content):
                logger.info(f"[dpbot] 引用图片文件已存在: {self.content}")
                return
            ...
            for file_type, cdn_url in download_urls:
                ...
                if result and isinstance(result, dict) and result.get("Code") == 200:
                    image_data = result.get("Data", {}).get("FileData")
                    if image_data and self._save_image_data(image_data):
                        logger.info(f"[dpbot] 引用图片下载成功: {self.content}")
                        return

            logger.warning("[dpbot] 所有引用图片下载尝试均失败")

        except Exception as e:
            logger.error(f"[dpbot] 引用图片下载异常: {e}")
    ...
    def _extract_video_info_from_xml(self, xml_content: str) -> dict:
        ...
        try:
            ...
            logger.error(f"[dpbot] 解析视频XML失败: {e}")
            return {}
    ...
    def download_video(self):
        ...
        try:
            ...
            if not self.client:
                logger.error("[dpbot] 没有客户端实例，无法下载视频")
                return

            content_dict = self.msg_data.get('Content', {})
            content_xml = content_dict.get('str', content_dict.get('string', ''))
            if not content_xml:
                logger.error("[dpbot] 没有找到视频XML内容")
                return
            ...
            if not video_info.get('aes_key') or not video_info.get('cdn_video_url'):
                logger.error("[dpbot] 视频CDN信息提取失败")
                return
            ...
            if result and result.get("Code") == 200:
                ...
                if video_data:
                    try:
                        ...
                        logger.info(f"[dpbot] 视频下载成功: {self.content}")
                    except Exception as e:
                        logger.error(f"[dpbot] 保存视频失败: {e}")
                else:
                    logger.error("[dpbot] API响应中没有FileData字段")
            else:
                logger.error(f"[dpbot] 视频下载API失败: {result}")
                
        except Exception as e:
            logger.error(f"[dpbot] 视频下载异常: {e}")
    ...
    def download_file(self):
        ...
        try:
            ...
            # 检查文件是否已存在，避免重复下载
            if os.path.exists(self.content):
                logger.info(f"[dpbot] 文件已存在: {self.content}")
                return
            ...
            if result and result.get("Code") == 200:
                ...
                if file_data:
                    try:
                        ...
                        logger.info(f"[dpbot] 文件下载成功: {self.content} ({file_info.get('title')})")
                    except Exception as e:
                        logger.error(f"[dpbot] 保存文件失败: {e}", exc_info=True)
                else:
                    logger.error("[dpbot] API响应中没有FileData字段")
            else:
                logger.error(f"[dpbot] 文件下载API失败: {result}")
                
        except Exception as e:
            logger.error(f"[dpbot] 文件下载异常: {e}", exc_info=True)
    ...
    def _clear_image_cache_for_text(self):
        ...
        try:
            ...
            if session_id in memory.USER_IMAGE_CACHE:
                ...
                logger.debug(f"[dpbot] 普通文本消息清理了图片缓存: session_id={session_id}")
                
        except Exception as e:
            logger.error(f"[dpbot] 清理图片缓存异常: {e}")
    ...
    def _is_non_user_message(self, msg_source: str, from_user_id, content: str = '', msg_type: int = 0) -> bool:
        ...
        if from_user_id in special_accounts or from_user_id.startswith("gh_"):
            logger.debug(f"[dpbot] non-user message detected by sender id: {from_user_id}")
            return True
        ...
        if any(indicator in msg_source for indicator in non_user_indicators):
            logger.debug(f"[dpbot] non-user message detected by msg_source indicators")
            return True
        ...
        if msg_type == 10000:
            ...
            if any(notification in content for notification in system_notifications):
                logger.debug(f"[dpbot] non-user message detected by system notification content: {content}")
                return True
    ...
    def download_refer_image_for_multimodal(self):
        ...
        try:
            ...
            if image_path and os.path.exists(image_path):
                ...
                logger.info(f"[dpbot] 引用图片已加入多模态缓存: session_id={session_id}")
            
            # 恢复原始的文本内容
            self.content = original_content
            
        except Exception as e:
            logger.error(f"[dpbot] 引用图片多模态处理异常: {e}")
    ...
    def download_refer_file_for_multimodal(self):
        ...
        try:
            ...
            if file_path and os.path.exists(file_path):
                ...
                logger.info(f"[dpbot] 引用文件已加入缓存: session_id={session_id}, file={refer_file_info.get('title')}")
            
            # 恢复原始的文本内容
            self.content = original_content
            
        except Exception as e:
            logger.error(f"[dpbot] 引用文件多模态处理异常: {e}")
    ...
    def _parse_refer_file_content(self, refer_content: str) -> dict:
        ...
        try:
            ...
            if appmsg is None:
                logger.debug("[dpbot] 引用内容中没有找到appmsg元素")
                return {}
            
            # 获取消息类型
            msg_type_node = appmsg.find('type')
            if msg_type_node is None or msg_type_node.text != '6':
                logger.debug(f"[dpbot] 引用消息类型不是文件: {msg_type_node.text if msg_type_node is not None else 'None'}")
                return {}
            ...
            if appattach is None:
                logger.debug("[dpbot] 引用文件消息中没有找到appattach元素")
                return {}
            ...
            logger.debug(f"[dpbot] 成功解析引用文件信息: {file_info['title']}")
            return file_info
            
        except ET.ParseError as e:
            logger.warning(f"[dpbot] 解析引用文件XML失败: {e}")
            return {}
        except Exception as e:
            logger.error(f"[dpbot] 解析引用文件内容异常: {e}")
            return {}